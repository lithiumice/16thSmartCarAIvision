...
{
    enum finsh_token_type token;
    struct finsh_node* finsh_or;
    struct finsh_node* assign;

    finsh_or = proc_inclusive_or_expr(self);
    ...
    if (token == finsh_token_type_assign)
    {
        ...
        return make_sys_node(FINSH_NODE_SYS_ASSIGN, finsh_or, assign);
    }
    else finsh_token_replay(&(self->token));
...
{
    enum finsh_token_type token;
    struct finsh_node* finsh_xor;
    struct finsh_node* xor_new;

    finsh_xor = proc_exclusive_or_expr(self);
    ...
    while ( token == finsh_token_type_or )
    {
        ...
        else finsh_xor = make_sys_node(FINSH_NODE_SYS_OR, finsh_xor, xor_new);

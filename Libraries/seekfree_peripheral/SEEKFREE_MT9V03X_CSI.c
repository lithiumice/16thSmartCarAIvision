...
int16 MT9V03X_CFG_CSI[CONFIG_FINISH][2] =
    {
        ...
        {EXP_TIME, 40},           //曝光时间          摄像头收到后会自动计算出最大曝光时间，如果设置过大则设置为计算出来的最大曝光值
...
//void csi_seekfree_sendimg_03x_cdc(uint8 *image, uint16 width, uint16 height)
//{
//    usb_cdc_send_char(0x00);
//    usb_cdc_send_char(0xff);
//    usb_cdc_send_char(0x01);
//    usb_cdc_send_char(0x01);                  //发送命令

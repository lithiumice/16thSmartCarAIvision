...
void w25qxxjv_init(void)
{
    ...
    if (status != kStatus_Success)
        assert(0); //获取ID失败 则进入断言
    if (0x16 != vendorID)
        assert(0); //ID校验失败 则进入断言
}
...
uint8 w25qxxjv_write_page(uint32 sector_num, uint32 page_num, const void *buf, uint8 len)
{
    ...
    isr = DisableGlobalIRQ();
    status = flexspi_nor_flash_page_program(W25QXXJV_FLEXSPI, sector_num * W25QXXJV_SECTOR_SIZE + W25QXXJV_PAGE_SIZE * page_num, (const uint32_t *)buf, len * 4);
    EnableGlobalIRQ(isr);

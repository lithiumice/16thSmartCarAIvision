...
void speed_loop_thread(void *parameter)
{
    ...
    if (chassis_normal == f1.chassis_update_type)
    {
        ...
        chassis.left_wheel.current_pid.target = (speed_loop.output + dir_inner.output);
        chassis.right_wheel.current_pid.target = (speed_loop.output - dir_inner.output);
    }
    else if (chassis_stop == f1.chassis_update_type)
    {
    }

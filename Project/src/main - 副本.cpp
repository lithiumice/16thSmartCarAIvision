...
void adc_bat_thread(void *parameters)
{
    ...
    // {
    //     dymparam.s_max_speed = 340;
    //     dymparam.s_min_speed = 280;
    //     dymparam.fore_min = 67;
    //     // steer.steer_fuzzy_kp_p=6.8;
    //     // steer.steer_kp=4.2;
    //     // steer.steer_kd=50;
    // }
    // else
    // {
    //     // dymparam.s_max_speed = 350;
    //     // dymparam.s_min_speed = 250;
    //     // dymparam.fore_min = 65;
    //     dymparam.s_max_speed = 340;
    //     dymparam.s_min_speed = 270;
    //     dymparam.fore_min = 62;
    //     steer.steer_fuzzy_kp_p=7.2;
    //     steer.steer_kp=3.5;
    //     steer.steer_kd=50;
    // }
...
void pos_loop_loop()
{
    while (1)
    {
        ...
        // )
        if (((chassis.all_distance >= pos_loop.target) && pos_loop.target > 0) ||
            ((chassis.all_distance <= pos_loop.target) && pos_loop.target < 0) ||
            ((fabs(chassis.all_distance - pos_loop.target) <= 3) && pos_loop.target == 0))
        {
            break;
        }
        rt_thread_mdelay(20);
...
int main(void)
{
    ...
    pos_loop = pos_pid(4.0, 0, 90.0, 100, -100);
    ...
    while (1)
    {
        ...
        img_proc_c.gray_to_binary((uint8 **)gray_120_160_image, (uint8 **)bin_120_160_image, 120, 160, 2);
        ...
        else if (f2.sch_sm_state == regco)
        {
            ...
            if (f1.finish_regco_flag == 1 && f1.regco_num != -1)
            {
                ...
                //     f1.regco_num = -1;
                // }
                if (f1.regco_num == 2) //偶数，前进30cm再后退
                {
                    // pos_loop.target = 30;
                    // pos_loop_loop();
